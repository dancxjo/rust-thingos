    ...
    for raw_line in input.lines() {
            ...
            if let Some(vendor) = vendors.last_mut() {
                if let Some((id_hex, name)) = split_id_and_name(rest.trim_start()) {
                    if let Ok(device_id) = parse_hex_id(id_hex) {
                        vendor.devices.push(Device { id: device_id, name: name.to_string() });
                    }
                }
            }
            continue;
        ...
        if let Some((id_hex, name)) = split_id_and_name(line.trim_start()) {
            if let Ok(vendor_id) = parse_hex_id(id_hex) {
                vendors.push(Vendor { id: vendor_id, name: name.to_string(), devices: Vec::new() });
            }
        }
    }
...
pub fn build_tables(vendors: &[Vendor]) -> GeneratedTables {
    ...
    for vendor in vendors_sorted.iter() {
        ...
        vendor_entries.push(VendorEntry { vendor: vendor.id, name_off: v_off, name_len: v_len });
    ...
    GeneratedTables { strings, vendors: vendor_entries, devices: device_entries }
}

pub fn lookup_vendor<'a>(tables: &'a GeneratedTables, vendor: u16) -> Option<&'a str> {
    tables.vendors.binary_search_by_key(&vendor, |v| v.vendor).ok().and_then(|idx| {
...
pub fn render_rust(tables: &GeneratedTables, snapshot: &SnapshotInfo, mode: Mode) -> String {
    ...
    writeln!(&mut out, "// @generated by tools/pciids. Do not edit by hand.").unwrap();
    writeln!(
        &mut out,
    ...
    .unwrap();
    writeln!(&mut out, "pub const PCI_IDS_MODE: &str = \"{}\";", mode.as_str()).unwrap();
    writeln!(&mut out, "pub const PCI_IDS_NAME_SOURCE: &str = \"{}\";", name_source(snapshot))
        .unwrap();
    writeln!(&mut out, "pub const PCI_VENDOR_COUNT: usize = {};", tables.vendors.len()).unwrap();
    writeln!(&mut out, "pub const PCI_DEVICE_COUNT: usize = {};", tables.devices.len()).unwrap();
...
fn push_name(strings: &mut Vec<u8>, name: &str) -> (u32, u16) {
    assert!(strings.len() <= u32::MAX as usize, "PCI string table overflow");
...
\t1000\tV1\r\n";
    ...
    fn filters_allowlist_edge_cases() {
        let vendors = vec![Vendor { id: 0x9999, name: "Unknown".to_string(), devices: Vec::new() }];
