Feature: VFS Extended Attributes
  ...
  Scenario: Round trip with mutable attributes
    ...
    Then the command output should contain "provider.name"
    And the command output should contain "provider.is_demo"
    And the command output should contain "provider.requests"
    ...
    Then the command output should contain "type=Utf8 len=14"
    And the command output should contain "value=This_is_a_test"
    When I type "attr_list /run/cookbook/hello.txt" on the serial console
    Then the command output should contain "user.comment"
