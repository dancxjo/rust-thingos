...
fn fetch_fonts(assets: &Path) -> Result<()> {
    ...
    let iosevka_dest = fonts_dir.join("Iosevka-Regular.ttf");
    if !iosevka_dest.exists() {
        println!("    Fetching Iosevka-Regular.ttf...");
        let zip_path = fonts_dir.join("temp_iosevka.zip");
        download_file(
            "https://github.com/be5invis/Iosevka/releases/download/v34.4.0/PkgTTF-Iosevka-34.4.0.zip",
            &zip_path,
        )?;
...
fn fetch_lucide_icons(assets: &Path) -> Result<()> {
    println!("==> Fetching Lucide Icons...");
    let icons_dir = assets.join("icons").join("lucide");
    fs::create_dir_all(&icons_dir)?;

    let boot_icon_names = [
        ("monitor", "ICON_FRAMEBUFFER"),
        ("memory-stick", "ICON_MEMORY"),
        ("box", "ICON_ALLOCATOR"),
        ("presentation", "ICON_DISPLAY"),
        ("zap", "ICON_SIMD"),
        ("dices", "ICON_ENTROPY"),
        ("layers", "ICON_TASKING"),
        ("hard-drive", "ICON_VFS"),
        ("circuit-board", "ICON_PCI"),
        ("plug", "ICON_LEGACY_DEVICES"),
        ("timer", "ICON_TIMER"),
        ("cpu", "ICON_SMP"),
        ("info", "ICON_BOOT_INFO"),
        ("blocks", "ICON_MODULES"),
        ("rocket", "ICON_INIT"),
        ("list-todo", "ICON_SCHEDULER"),
    ];
    let window_icon_names =
        ["minus", "chevron-up", "chevron-down", "square", "fullscreen", "minimize-2", "x"];

    let mut generated_rs = String::new();
    generated_rs.push_str("// Auto-generated by xtask fetch\n\n");

    for name in window_icon_names {
        let svg_path = icons_dir.join(format!("{}.svg", name));
        ensure_lucide_svg(name, &svg_path)?;
    }

    for (name, const_name) in boot_icon_names {
        let svg_path = icons_dir.join(format!("{}.svg", name));
        let svg_data = ensure_lucide_svg(name, &svg_path)?;
    ...
    let dest_rs = root.join("thingos/kernel/src/boot_progress_icons.rs");
    fs::write(&dest_rs, generated_rs)?;
    println!("    Generated boot_progress_icons.rs");

    Ok(())
}

fn ensure_lucide_svg(name: &str, svg_path: &Path) -> Result<String> {
    if !svg_path.exists() {
    ...
    let normalized = svg_data.replace("currentColor", "#000000");
    if normalized != svg_data {
        fs::write(svg_path, &normalized)?;
        svg_data = normalized;
    }
    Ok(svg_data)
}
