...
pub fn enumerate(rsdp_phys: u64, hhdm: u64) -> Vec<AcpiEntry> {
    // SAFETY: HHDM covers all physical RAM by the time processes run.
    unsafe { do_enumerate(rsdp_phys, hhdm) }
}

unsafe fn do_enumerate(rsdp_phys: u64, hhdm: u64) -> Vec<AcpiEntry> {
    let rsdp_virt = rsdp_phys.wrapping_add(hhdm);

    // Verify signature.
    let sig = unsafe { core::ptr::read(rsdp_virt as *const [u8; 8]) };
    if &sig != RSDP_SIG {
...
unsafe fn walk_rsdt(phys: u64, hhdm: u64) -> Vec<AcpiEntry> {
    ...
    let base_virt = phys.wrapping_add(hhdm);
    let mut out = Vec::new();
    for i in 0..entry_count {
        let ptr = (base_virt + SDT_HEADER_LEN as u64 + (i as u64) * 4) as *const u32;
        let addr = unsafe { core::ptr::read_unaligned(ptr) } as u64;
        if addr == 0 {
            continue;
        }
...
unsafe fn walk_xsdt(phys: u64, hhdm: u64) -> Vec<AcpiEntry> {
    ...
    let mut out = Vec::new();
    for i in 0..entry_count {
        let ptr = (base_virt + SDT_HEADER_LEN as u64 + (i as u64) * 8) as *const u64;
        let addr = unsafe { core::ptr::read_unaligned(ptr) };
        if addr == 0 {
            continue;
        }
        if let Some(entry) = unsafe { read_full_table(addr, hhdm) } {
...
unsafe fn read_sdt_header(phys: u64, hhdm: u64) -> Option<[u8; SDT_HEADER_LEN]> {
    ...
    unsafe {
        core::ptr::copy_nonoverlapping(virt as *const u8, buf.as_mut_ptr(), SDT_HEADER_LEN);
    }
    Some(buf)
}
...
unsafe fn read_full_table(phys: u64, hhdm: u64) -> Option<AcpiEntry> {
    ...
    let slice = unsafe { core::slice::from_raw_parts(virt as *const u8, total) };
