...
fn resolve_ext(path: &str, follow_final: bool, depth: usize) -> SysResult<Arc<dyn VfsNode>> {
    ...
    match crate::vfs::mount::lookup(&normalised) {
        Ok(node) => {
            crate::ktrace!("resolve_ext: fast-path lookup success for '{}'", normalised);
            if follow_final {
                let stat = node.stat()?;
                crate::ktrace!("resolve_ext: fast-path stat success mode=0o{:o}", stat.mode);
                if stat.is_symlink() {
    ...
    crate::ktrace!("resolve_ext: falling back to slow-path walk for '{}'", normalised);
...
fn walk_path(path: &str, follow_final: bool, depth: usize) -> SysResult<Arc<dyn VfsNode>> {
    ...
    for (i, component) in components.iter().enumerate() {
        ...
        crate::ktrace!("walk_path: component='{}' current_path='{}'", component, current_path);
