...
/// Standard Wayland-style IPC header (8 bytes)
#[repr(C, packed)]
#[derive(Clone, Copy)]
pub struct WaylandIpcHeader {
    pub object_id: u32,
    pub size_and_opcode: u32, // Upper 16 bits: total size, Lower 16 bits: opcode
}

impl WaylandIpcHeader {
    pub const SIZE: usize = 8;
    pub fn opcode(&self) -> u16 {
        (self.size_and_opcode & 0xFFFF) as u16
    }
    pub fn payload_len(&self) -> u32 {
        (self.size_and_opcode >> 16).saturating_sub(8)
    }
    ...
    pub fn to_bytes(&self) -> [u8; 8] {
        ...
        buf[4..8].copy_from_slice(&self.size_and_opcode.to_le_bytes());
        buf
    }
    pub fn from_bytes(bytes: &[u8; 8]) -> Self {
        Self {
            object_id: u32::from_le_bytes(bytes[0..4].try_into().unwrap()),
            size_and_opcode: u32::from_le_bytes(bytes[4..8].try_into().unwrap()),
        }
    }
}

/// Wayland IPC absolute pointer motion payload
#[repr(C, packed)]
#[derive(Clone, Copy)]
pub struct WaylandPointerMotion {
    pub x: i32,
    pub y: i32,
}

impl WaylandPointerMotion {
    pub const SIZE: usize = 8;
    pub fn to_bytes(&self) -> [u8; 8] {
        let mut buf = [0u8; 8];
...
/// Wayland IPC pointer button payload
#[repr(C, packed)]
#[derive(Clone, Copy)]
pub struct WaylandPointerButton {
    pub button: u8,
    pub pressed: u8,
}

impl WaylandPointerButton {
    pub const SIZE: usize = 2;
    pub fn to_bytes(&self) -> [u8; 2] {
        [self.button, self.pressed]
    }
    pub fn from_bytes(bytes: &[u8; 2]) -> Self {
        Self { button: bytes[0], pressed: bytes[1] }
    }
}
