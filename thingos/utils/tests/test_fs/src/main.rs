...
fn test_create_write_read() -> Result<(), std::string::String> {
    ...
    {
        let mut f =
            std::fs::File::create(path).map_err(|e| std::alloc::format!("create: {}", e))?;
        f.write_all(content).map_err(|e| std::alloc::format!("write_all: {}", e))?;
    }

    // Reopen and read back.
    {
        let mut f = std::fs::File::open(path).map_err(|e| std::alloc::format!("open: {}", e))?;
        let mut buf = std::vec::Vec::new();
        f.read_to_end(&mut buf).map_err(|e| std::alloc::format!("read_to_end: {}", e))?;
        if buf != content {
    ...
    // Cleanup.
    std::fs::remove_file(path).map_err(|e| std::alloc::format!("remove_file: {}", e))?;
...
fn test_seek() -> Result<(), std::string::String> {
...

    {
        ...
        f.seek(SeekFrom::Start(4)).map_err(|e| std::alloc::format!("seek(Start,4): {}", e))?;
        let mut buf = [0u8; 4];
        f.read_exact(&mut buf).map_err(|e| std::alloc::format!("read_exact after seek: {}", e))?;
        if &buf != b"4567" {
            return Err(std::alloc::format!(
                "seek+read mismatch: got {:?}, expected b\"4567\"",
                buf
            ));
        }
        ...
        if &buf2 != b"67" {
            return Err(std::alloc::format!(
                "seek(Current)+read mismatch: got {:?}, expected b\"67\"",
                buf2
            ));
        }

        // Seek from end: last 4 bytes.
        f.seek(SeekFrom::End(-4)).map_err(|e| std::alloc::format!("seek(End,-4): {}", e))?;
        let mut buf3 = [0u8; 4];
        f.read_exact(&mut buf3)
            .map_err(|e| std::alloc::format!("read_exact after seek(End): {}", e))?;
        if &buf3 != b"cdef" {
            return Err(std::alloc::format!(
                "seek(End)+read mismatch: got {:?}, expected b\"cdef\"",
                buf3
            ));
        }
    }
...
fn test_readdir() -> Result<(), std::string::String> {
    ...
    std::fs::create_dir(dir).map_err(|e| std::alloc::format!("create_dir: {}", e))?;

    // Create two files inside.
    let file_a = std::alloc::format!("{}/alpha.txt", dir);
    let file_b = std::alloc::format!("{}/beta.txt", dir);
    std::fs::File::create(&file_a).map_err(|e| std::alloc::format!("create alpha.txt: {}", e))?;
    std::fs::File::create(&file_b).map_err(|e| std::alloc::format!("create beta.txt: {}", e))?;
    ...
    // Cleanup.
    std::fs::remove_file(&file_a).map_err(|e| std::alloc::format!("remove alpha.txt: {}", e))?;
    std::fs::remove_file(&file_b).map_err(|e| std::alloc::format!("remove beta.txt: {}", e))?;
    std::fs::remove_dir(dir).map_err(|e| std::alloc::format!("remove_dir: {}", e))?;
...
fn test_metadata() -> Result<(), std::string::String> {
    ...
    {
        let mut f =
            std::fs::File::create(file_path).map_err(|e| std::alloc::format!("create: {}", e))?;
        f.write_all(content).map_err(|e| std::alloc::format!("write_all: {}", e))?;
    }

    // Verify file metadata.
    let meta =
        std::fs::metadata(file_path).map_err(|e| std::alloc::format!("metadata(file): {}", e))?;
    if !meta.is_file() {
    ...
    // Create a directory.
    std::fs::create_dir(dir_path).map_err(|e| std::alloc::format!("create_dir: {}", e))?;

    // Verify directory metadata.
    let dir_meta =
        std::fs::metadata(dir_path).map_err(|e| std::alloc::format!("metadata(dir): {}", e))?;
    if !dir_meta.is_dir() {
    ...
    // Cleanup.
    std::fs::remove_file(file_path).map_err(|e| std::alloc::format!("remove_file: {}", e))?;
    std::fs::remove_dir(dir_path).map_err(|e| std::alloc::format!("remove_dir: {}", e))?;
...
fn test_rename_remove() -> Result<(), std::string::String> {
    ...
    {
        let mut f =
            std::fs::File::create(old_path).map_err(|e| std::alloc::format!("create: {}", e))?;
        f.write_all(content).map_err(|e| std::alloc::format!("write_all: {}", e))?;
    }

    // Rename.
    std::fs::rename(old_path, new_path).map_err(|e| std::alloc::format!("rename: {}", e))?;
    ...
    // New path should exist with correct content.
    {
        ...
        let mut buf = std::vec::Vec::new();
        f.read_to_end(&mut buf).map_err(|e| std::alloc::format!("read after rename: {}", e))?;
        if buf != content {
            return Err(std::alloc::format!("content after rename mismatch: got {:?}", buf));
        }
    }

    // Delete the new file.
    std::fs::remove_file(new_path).map_err(|e| std::alloc::format!("remove_file: {}", e))?;
...
fn test_truncate() -> Result<(), std::string::String> {
    ...
    // Write 16 bytes.
    {
        let mut f =
            std::fs::File::create(path).map_err(|e| std::alloc::format!("create: {}", e))?;
        f.write_all(b"Hello, truncate!").map_err(|e| std::alloc::format!("write_all: {}", e))?;
    }
    ...
    // Open for writing and truncate to 5 bytes.
    {
        ...
        f.set_len(5).map_err(|e| std::alloc::format!("set_len(5): {}", e))?;
    }
    ...
    // Read back and verify.
    {
        ...
        f.read_to_end(&mut buf).map_err(|e| std::alloc::format!("read_to_end: {}", e))?;
        if buf != b"Hello" {
            return Err(std::alloc::format!("truncate: got {:?}, expected b\"Hello\"", buf));
        }
    }
...
fn test_chdir_relative_open() -> Result<(), std::string::String> {
    ...
    std::fs::create_dir(subdir).map_err(|e| std::alloc::format!("create_dir: {}", e))?;
    {
    ...
    let new_cwd = {
        ...
        alloc::string::String::from(
            core::str::from_utf8(&cwd_buf[..n]).map_err(|_| std::alloc::format!("non-UTF8 cwd"))?,
        )
    };
    if new_cwd != subdir {
        let _ = stem::syscall::vfs_chdir(&original_cwd);
        return Err(std::alloc::format!(
            "getcwd mismatch after chdir: expected {:?}, got {:?}",
            subdir,
            new_cwd
        ));
    }
    ...
    let result = (|| -> Result<(), std::string::String> {
        ...
        f.read_to_end(&mut buf).map_err(|e| std::alloc::format!("read_to_end: {}", e))?;
        if buf != content {
            return Err(std::alloc::format!("relative open content mismatch: {:?}", buf));
        }
        Ok(())
    })();
    ...
    // Cleanup.
    std::fs::remove_file(&abs_file).map_err(|e| std::alloc::format!("remove_file: {}", e))?;
    std::fs::remove_dir(subdir).map_err(|e| std::alloc::format!("remove_dir: {}", e))?;
...
fn test_getcwd_roundtrip() -> Result<(), std::string::String> {
    ...
    // chdir to /tmp.
    stem::syscall::vfs_chdir("/tmp").map_err(|e| std::alloc::format!("chdir /tmp: {:?}", e))?;

    // getcwd must now return "/tmp".
    let n2 = stem::syscall::vfs_getcwd(&mut buf)
        .map_err(|e| std::alloc::format!("getcwd (after /tmp): {:?}", e))?;
    let after = core::str::from_utf8(&buf[..n2]).map_err(|_| "non-UTF8 cwd after".to_string())?;
    if after != "/tmp" {
        let _ = stem::syscall::vfs_chdir(&original);
        return Err(std::alloc::format!(
            "getcwd after chdir /tmp: expected '/tmp', got {:?}",
            after
        ));
    }
...
fn test_mkdir_eexist() -> Result<(), std::string::String> {
    ...
    // First create should succeed.
    std::fs::create_dir(dir).map_err(|e| std::alloc::format!("create_dir (first): {}", e))?;
    ...
    match std::fs::create_dir(dir) {
        ...
        Err(e) => {
            let _ = std::fs::remove_dir(dir);
            return Err(std::alloc::format!("create_dir (second): unexpected error: {}", e));
        }
        Ok(()) => {
            let _ = std::fs::remove_dir(dir);
...
fn test_open_enoent() -> Result<(), std::string::String> {
    ...
    match std::fs::File::open(path) {
        ...
        Err(e) => Err(std::alloc::format!("open non-existent: expected NotFound, got {}", e)),
