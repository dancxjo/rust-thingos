...
use abi::auxv;
use stem::println;
use stem::syscall::{auxv_get, vfs_close, vfs_open};
...
fn collect_dt_needed(buf: &[u8]) -> alloc::vec::Vec<alloc::vec::Vec<u8>> {
    ...
    let phoff = match elf_u64(buf, 32) {
        Some(v) => v as usize,
        None => return result,
    };
    let phent = match elf_u16(buf, 54) {
        Some(v) => v as usize,
        None => return result,
    };
    let phnum = match elf_u16(buf, 56) {
        Some(v) => v as usize,
        None => return result,
    };
    ...
    for i in 0..phnum {
        ...
        match elf_u32(buf, off) {
            Some(2) => {
                // PT_DYNAMIC
                dyn_off = match elf_u64(buf, off + 8) {
                    Some(v) => v as usize,
                    None => 0,
                };
                dyn_sz = match elf_u64(buf, off + 32) {
                    Some(v) => v as usize,
                    None => 0,
                };
            }
    ...
    while ptr + 16 <= dyn_off + dyn_sz && ptr + 16 <= buf.len() {
        let tag = match elf_u64(buf, ptr) {
            Some(v) => v as i64,
            None => break,
        };
        let val = match elf_u64(buf, ptr + 8) {
            Some(v) => v,
            None => break,
        };
        if tag == 0 {
...
fn test_auxv_basics() {
    ...
    println!(
        "[test_dyn_loader] test_auxv_basics: PASS (AT_PAGESZ={}, AT_ENTRY=0x{:x})",
        pagesz, entry
    );
}
...
fn test_no_at_base_for_static_binary() {
    ...
    assert_eq!(base, 0, "AT_BASE should be 0 for a static binary, got 0x{:x}", base);
