...
fn count_fd(in_fd: u32) -> Counts {
    ...
    loop {
        match vfs_read(in_fd, &mut buf) {
            Ok(0) => break,
            Ok(count) => {
                stem::trace!("wc: read {} bytes from fd {}", count, in_fd);
                bytes += count;
                for &b in &buf[..count] {
                    if b == b'\n' {
                        lines += 1;
...
fn main(_arg: usize) -> ! {
    ...
    if file_args.is_empty() {
        stem::debug!("wc: counting from stdin (no files)");
        ...
        for path in &file_args {
            if *path == "-" {
                stem::debug!("wc: counting from stdin");
                let counts = count_fd(0);
                if multiple {
                ...
                stem::debug!("wc: counting file '{}'", path);
